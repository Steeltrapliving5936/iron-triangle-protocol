...
- `dispatch(role, text)`: focus the bound role window and insert a copy-ready prompt, or produce a manual-paste payload. UI focus or text insertion is **not** delivery: record `accepted: false` until the destination acknowledges.
